...
- **`receive-only`** (the only runnable posture today): the adapter durably handles inbound signed `PUT /transfers/{ID}` callbacks. The outbound leg — initiating quotes/transfers through `internal/mojaloop` `Client` — is **not wired**, so no quotes are ever initiated. Inbound `PUT /quotes/{ID}` callbacks are therefore authenticated exactly like transfer callbacks (FSPIOP JWS signature, key ID, source and destination verification; forged or misrouted callbacks fail closed with 401/403) and then answered with a truthful **501 `application/problem+json`** document instead of a silent 404 or a dishonest 202. The startup log line and the `/healthz` payload both report the configured mode so the posture is observable.
- **`full`** is parsed but the adapter **refuses to start** with it, because the outbound leg is not implemented. Enabling `full` still requires, at minimum: payee-party discovery/resolution for the receiving bank, a durable quote state machine (quote request → quote callback → transfer request) bound to the `DISBURSEMENT_PENDING` stage of `CVFFDisbursementWorkflow`, quote/transfer correlation persistence with idempotent replay, Temporal signal plumbing from the adapter into the workflow, and Ministry approval of the participant endpoints, certificates and rulebook.
