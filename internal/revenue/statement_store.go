...
// ---------------------------------------------------------------------------
// TigerBeetle mirror sync (W-CLOSE-FC): settlement_records is the
// materialized, queryable mirror of posted TB collection transfers.
// ---------------------------------------------------------------------------

// TBMirrorInput is one posted TigerBeetle collection transfer to mirror.
// The values are taken from the ledger entry verbatim — the sync never
// fabricates references: BankReference/PayerRef are derived deterministically
// from the transfer identity when the ledger carries no reference
// convention, so recon honestly flags them for resolution.
type TBMirrorInput struct {
	TBTransferID  string `json:"tbTransferId"` // hex transfer id (required)
	AmountMinor   int64  `json:"amountMinor"`
	Currency      string `json:"currency"` // from the configured ledger->currency map
	BankReference string `json:"bankReference"`
	PayerRef      string `json:"payerRef"`
	ValueDate     string `json:"valueDate"` // YYYY-MM-DD (transfer timestamp, UTC)
}

// Validate fails closed on a malformed mirror record.
func (input TBMirrorInput) Validate() error {
	if strings.TrimSpace(input.TBTransferID) == "" || len(input.TBTransferID) > 64 {
		return errors.New("tbTransferId is required")
	}
	if input.AmountMinor <= 0 {
		return errors.New("amountMinor must be positive")
	}
...
// RecordTBSettlement upserts one mirrored TigerBeetle collection transfer
// and advances the sync cursor in the SAME transaction, so a crash between
// transfers replays idempotently. created is false when the transfer was
// already mirrored (tb_transfer_id UNIQUE — replay is a no-op; the cursor
// still advances). The mirror is recorded under the tb-sync system actor
// and publishes revenue.settlement.recorded only on first insert.
func (store *Store) RecordTBSettlement(ctx context.Context, input TBMirrorInput, syncKey string, observedTimestamp uint64) (created bool, err error) {
	if err := input.Validate(); err != nil {
		return false, err
	}
	if syncKey == "" {
		return false, errors.New("sync key is required")
	}
	_, requestHash, err := canonicalJSON(input)
	if err != nil {
		return false, err
	...
	tx, err := store.pool.Begin(ctx)
	if err != nil {
		return false, fmt.Errorf("begin mirror transaction: %w", err)
	}
	defer tx.Rollback(ctx)
	settlementID := uuid.NewString()
	var inserted string
	err = tx.QueryRow(ctx,
		`INSERT INTO settlement_records (settlement_id, idempotency_key, request_hash, debit_note_id,
		 ...
		 ON CONFLICT (tb_transfer_id) WHERE tb_transfer_id IS NOT NULL AND tb_transfer_id <> '' DO NOTHING
		 RETURNING settlement_id`,
		settlementID, "tb-sync:"+input.TBTransferID, requestHash, input.BankReference,
		input.TBTransferID, input.AmountMinor, input.Currency, input.PayerRef,
		input.ValueDate, actor).Scan(&inserted)
	if err != nil && !errors.Is(err, pgx.ErrNoRows) {
		return false, fmt.Errorf("mirror settlement %s: %w", input.TBTransferID, err)
	}
	created = err == nil
	if created {
		if err := insertOutbox(ctx, tx, settlementID, "revenue.settlement.recorded", map[string]any{
			"settlementId": settlementID, "bankReference": input.BankReference,
			"tbTransferId": input.TBTransferID, "amountMinor": input.AmountMinor,
			"currency": input.Currency, "actor": actor,
		}); err != nil {
			return false, err
		}
	}
	if _, err := tx.Exec(ctx,
		`INSERT INTO settlement_sync_state (sync_key, last_timestamp)
		 VALUES ($1, $2)
		 ON CONFLICT (sync_key) DO UPDATE
		 SET last_timestamp = GREATEST(settlement_sync_state.last_timestamp, EXCLUDED.last_timestamp),
		     updated_at = now()`,
		syncKey, int64(observedTimestamp)); err != nil {
		return false, fmt.Errorf("advance sync cursor %s: %w", syncKey, err)
	}
	if err := tx.Commit(ctx); err != nil {
		return false, fmt.Errorf("commit mirror: %w", err)
	}
	if created {
		recordMoneyOp(ctx, "settlement.tb_mirrored")
	}
	return created, nil
}

// AdvanceSyncCursor moves the cursor past timestamps that carry no mirrored
// row (pending reservations and voids are skipped deterministically, so a
// crash before the bump simply re-reads and re-skips them).
func (store *Store) AdvanceSyncCursor(ctx context.Context, syncKey string, timestamp uint64) error {
	if syncKey == "" {
		return errors.New("sync key is required")
	}
	if _, err := store.pool.Exec(ctx,
		`INSERT INTO settlement_sync_state (sync_key, last_timestamp)
		 ...
		 SET last_timestamp = GREATEST(settlement_sync_state.last_timestamp, EXCLUDED.last_timestamp),
		     updated_at = now()`,
		syncKey, int64(timestamp)); err != nil {
		return fmt.Errorf("advance sync cursor %s: %w", syncKey, err)
	}
	return nil
}

// SyncCursor loads the last mirrored TigerBeetle timestamp for one sync key
// (0 when the sync never ran).
func (store *Store) SyncCursor(ctx context.Context, syncKey string) (uint64, error) {
	var cursor int64
	err := store.pool.QueryRow(ctx,
		`SELECT last_timestamp FROM settlement_sync_state WHERE sync_key = $1`, syncKey).Scan(&cursor)
	if errors.Is(err, pgx.ErrNoRows) {
		return 0, nil
	}
	if err != nil {
		return 0, fmt.Errorf("load sync cursor %s: %w", syncKey, err)
	}
