...
func requireSixReplicaAuthorization(t *testing.T) []string {
	t.Helper()
	if os.Getenv("TB_QUORUM_TEST_ENABLED") != "true" {
		t.Fatal("TB_QUORUM_TEST_ENABLED=true is required")
	}
	if strings.TrimSpace(os.Getenv("TB_QUORUM_AUTHORIZATION_REF")) == "" {
		t.Fatal("TB_QUORUM_AUTHORIZATION_REF is required")
	}
	if strings.TrimSpace(os.Getenv("TB_QUORUM_CASE")) == "" {
		t.Fatal("TB_QUORUM_CASE is required")
	}
	endpoints := strings.Split(requiredEnv(t, "TIGERBEETLE_REPLICA_ADDRESSES"), ",")
	if len(endpoints) != 6 {
		t.Fatalf("expected exactly six TigerBeetle replica addresses, got %d", len(endpoints))
	}
	for _, endpoint := range endpoints {
		if strings.TrimSpace(endpoint) == "" {
			t.Fatal("blank TigerBeetle replica address")
		}
	}
	return endpoints
}
...
func TestSixReplicaQuorum(t *testing.T) {
	...
	client, err := tigerbeetle.NewClient(cluster, endpoints)
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
	...
	if err := service.CreateAccount(a, true); err != nil {
		t.Fatal(err)
	}
	if err := service.CreateAccount(b, true); err != nil {
		t.Fatal(err)
	}
	if _, found, err := service.LookupTransfer(tigerbeetle.ToUint128(0)); err != nil || found {
		t.Fatalf("unexpected baseline ledger result found=%v err=%v", found, err)
	}
	_ = ctx
}
...
func TestSixReplicaQuorumReconciliation(t *testing.T) {
	...
	if strings.TrimSpace(os.Getenv("DATABASE_URL")) == "" {
		t.Fatal("DATABASE_URL is required for reconciliation")
	}
	if strings.TrimSpace(os.Getenv("MIGRATION_PATH")) == "" {
		t.Fatal("MIGRATION_PATH is required for reconciliation")
	}
