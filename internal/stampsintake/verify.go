...
// TrustedKeysEnv carries the trusted tax-stamps signing keys as a
...
const TrustedKeysEnv = "STAMPS_INTAKE_TRUSTED_KEYS"
...
var (
	...
	ErrUntrustedKey = errors.New("stamps event signer key is not trusted")
	// ErrSignature rejects a JWS whose EdDSA signature does not verify or
	// whose payload is not the canonical envelope.
	ErrSignature = errors.New("stamps event signature does not verify")
)

// Verifier checks stamps-event provenance signatures against the trusted
// tax-stamps key set.
...
func NewVerifier(trusted map[string]ed25519.PublicKey) (*Verifier, error) {
	if len(trusted) == 0 {
		return nil, errors.New("stamps intake trusted key set is required (fail-closed)")
	}
	keys := make(map[string]ed25519.PublicKey, len(trusted))
	for kid, key := range trusted {
		if strings.TrimSpace(kid) == "" || len(kid) > 128 {
			return nil, errors.New("stamps intake trusted key id is invalid")
		}
		if len(key) != ed25519.PublicKeySize {
			return nil, fmt.Errorf("stamps intake trusted key %q is not an Ed25519 public key", kid)
		}
...
//
// The scheme mirrors the producer (tax-stamps crypto/eddsa.jws_sign): EdDSA
// over the JCS-canonical envelope with provenance.signature excluded; the
// payload segment must reproduce byte-for-byte so no reformatting can alter
// the signed content.
func (verifier *Verifier) VerifyAndParse(raw []byte, topic string) (StampEvent, error) {
	if verifier == nil {
		return StampEvent{}, errors.New("stamps intake verifier is required (fail-closed)")
	}
	event, err := parseEvent(raw, topic)
	if err != nil {
		return StampEvent{}, err
	}
