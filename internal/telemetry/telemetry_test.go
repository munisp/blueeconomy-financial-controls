...
func TestLoadConfigDefaultsToDisabled(t *testing.T) {
	...
	config, err := LoadConfig("blueeconomy-financial-controls")
	if err != nil {
	...
	if config.ServiceName != "blueeconomy-financial-controls" {
...
func TestLoadConfigFailsClosedOnMalformedValues(t *testing.T) {
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if _, err := LoadConfig("blueeconomy-financial-controls"); err == nil {
...
func TestMiddlewareCreatesSpanWithRouteAndStatus(t *testing.T) {
	...
	mux.HandleFunc("POST /v1/financial-intents/{intent_id}/approve", func(writer http.ResponseWriter, request *http.Request) {
	...
	request := httptest.NewRequest(http.MethodPost, "/v1/financial-intents/intent-1/approve", nil)
	...
	span := spans[0]
	if span.Name() != "POST /v1/financial-intents/{intent_id}/approve" {
		t.Fatalf("span must be renamed to the matched route, got %q", span.Name())
	}
	attributes := span.Attributes()
	assertAttribute(t, attributes, "http.route", "POST /v1/financial-intents/{intent_id}/approve")
