// revenue-intake consumes the port-interoperability tariff engine's signed
// revenue assessment events from finance.revenue-assessments.v1 and lands
// them as assessment-side reconciliation records. Every message is
...
import (
	...
	"github.com/munisp/blueeconomy-financial-controls/internal/revenueintake"
	"github.com/munisp/blueeconomy-financial-controls/internal/telemetry"
)

func main() {
	logger := log.New(os.Stdout, "revenue-intake: ", log.LstdFlags|log.LUTC)
...
func run(logger *log.Logger) error {
	...
	groupID, err := required("REVENUE_INTAKE_GROUP_ID")
	if err != nil {
		return err
	}
	...
	store, err := revenueintake.NewStore(pool)
	if err != nil {
		return err
	}
	verifier, err := revenueintake.VerifierFromEnv()
	if err != nil {
		return err
	}
	consumer, err := revenueintake.NewConsumer(revenueintake.Config{
		Brokers: strings.Split(brokersRaw, ","),
		GroupID: groupID,
		Topic:   revenueintake.Topic,
	}, verifier, store, logger)
	if err != nil {
		return err
	}
	defer consumer.Close()
	logger.Printf("consuming %s (group %s)", revenueintake.Topic, groupID)
	return consumer.Run(ctx)
}

func setupTelemetry(ctx context.Context) (*telemetry.Telemetry, error) {
	config, err := telemetry.LoadConfig("revenue-intake")
	if err != nil {
